  ...
  <div>
    <div class="page-header page-header-small">
      ...
      <div class="content-center">
        <div class="container">
          <h1 class="title">ระบบการแจ้งข่าวสารภายใน</h1>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getUserData: function() {
      ...
      let mytoken = myStorage.getItem('token');
      if (uid){
         axios
        .get('/api/users/'+uid, { headers: {"Authorization" : `Bearer ${mytoken}`} })
        .then(response => {
          //console.log(response);
          //self.$set(this, "user", response.data.email);
        })
        .catch(errors => {
          console.log(errors);
          myStorage.clear();
